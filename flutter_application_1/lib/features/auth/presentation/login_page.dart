...
// 登入頁面 (Login Page)
// ====================================================================
// 這個檔案包含使用者登入功能，包含帳號密碼登入和Google登入
...
// 登入頁面狀態管理類別 - 處理登入功能和畫面狀態
class _LoginPageState extends State<LoginPage> {
  // 文字輸入控制器:管理帳號輸入框的文字內容
  ...
  // 密碼可見性控制:true表示密碼以明文顯示，false表示以星號顯示
  bool _isPasswordVisible = false;

  // 一般登入載入狀態:true表示正在登入中，會顯示載入中的圖示
  bool _isLoading = false;

  // Google登入載入狀態:true表示正在用Google帳號登入中
  bool _isGoogleLoading = false;

  // Google登入設定物件 - 設定需要的權限
  final GoogleSignIn _googleSignIn = GoogleSignIn(
    scopes: ['email', 'profile'], // 要求取得使用者的電子郵件和基本資料
  );

  /// 處理一般登入流程 - 檢查表單並執行登入功能
  void _handleLogin() async {
    // 驗證表單輸入:檢查所有必填欄位是否填寫正確
    if (!_formKey.currentState!.validate()) return;

    // 設定載入狀態為true:讓畫面顯示載入中的圖示
    setState(() {
      _isLoading = true;
    });

    // 模擬登入過程:在實際應用中這裡會連線到伺服器進行身分驗證
    ...
    // 跳到主畫面:用pushReplacement替換目前頁面(使用者按返回鍵不會回到登入頁面)
  ...
  /// Google登入處理方法 - 執行用Google帳號登入的流程
  Future<void> _handleGoogleSignIn() async {
    // 設定Google登入載入狀態:顯示載入中的圖示
    setState(() {
      _isGoogleLoading = true;
    });

    try {
      // 執行Google登入:開啟Google登入視窗讓使用者選擇帳號
      ...
      if (mounted) {
        // 檢查Widget是否還在
        setState(() {
          _isGoogleLoading = false; // 隱藏Google登入載入中的圖示
        });
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.grey[100], // 頁面背景設為淺灰色
      body: SafeArea(
        // 確保內容不會被手機上方狀態列或下方按鈕遮住
        child: Center(
          // 把所有內容置中顯示
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: Colors.blue, // 背景顏色為藍色
                      borderRadius: BorderRadius.circular(60), // 圓角半徑60讓它變圓形
                    ),
                    child: const Icon(
                      Icons.person, // 使用人物圖示
                      size: 60, // 圖示大小
                      color: Colors.white, // 圖示顏色為白色
                    ),
                  ),
                  ...
                  const Text(
                    ...
                    style: TextStyle(
                      fontSize: 28, // 字體大小
                      fontWeight: FontWeight.bold, // 字體粗細為粗體
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: const Icon(Icons.person_outline), // 前面的圖示
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(12), // 圓角邊框
                      ),
                      filled: true, // 啟用填充色
                      fillColor: Colors.white, // 填充顏色為白色
                    ),
                    // 輸入驗證函數(檢查輸入是否正確)
                    validator: (value) {
                      if (value == null || value.isEmpty) {
                        return '請輸入帳號'; // 如果沒填就顯示這個錯誤訊息
                      }
                      if (value.length < 3) {
                        return '帳號至少需要3個字元'; // 如果太短就顯示這個錯誤訊息
                      }
                      return null; // 驗證通過回傳null
                    },
                  ),
